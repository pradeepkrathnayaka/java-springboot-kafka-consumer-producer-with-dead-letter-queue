...
public class KafkaConfig {
	...
	public NewTopic mainTopic() {
		return TopicBuilder.name(mainTopic).partitions(3).replicas(1) // ≥3 replicas in production
				.config("min.insync.replicas", "1").build();
	}

	@Bean
	public NewTopic dltTopic() {
		return TopicBuilder.name(dltTopic).partitions(3).replicas(1)
	...
	public ConsumerFactory<String, Object> consumerFactory() {
		...
		jsonDeserializer.configure(Map.of(
				...
				JacksonJsonDeserializer.TRUSTED_PACKAGES, "com.rmpk.kconsu.model",
				// Honour the __TypeId__ header set by the producer
				JacksonJsonDeserializer.USE_TYPE_INFO_HEADERS, "true"), false /* isKey */);

		// --- Wrap both deserializers with ErrorHandlingDeserializer (FIX #1) ------
		ErrorHandlingDeserializer<String> keyDeser = new ErrorHandlingDeserializer<>(new StringDeserializer());
		ErrorHandlingDeserializer<Object> valueDeser = new ErrorHandlingDeserializer<>(jsonDeserializer);
	...
	public DeadLetterPublishingRecoverer deadLetterPublishingRecoverer(KafkaTemplate<String, Object> kafkaTemplate) {
		...
		BiFunction<ConsumerRecord<?, ?>, Exception, TopicPartition> destinationResolver = (record, ex) -> {
			log.error("🔀 Routing to DLT → Topic: {}, Key: {}, Exception: {}", record.topic(), record.key(),
					ex.getMessage());
	...
	public DefaultErrorHandler defaultErrorHandler(DeadLetterPublishingRecoverer recoverer) {
		...
		errorHandler.addNotRetryableExceptions(NonRetryableException.class,
				...
				org.apache.kafka.common.errors.SerializationException.class, ClassCastException.class,
				IllegalArgumentException.class // covers trusted-packages failures
		);

		// ── Retryable: transient failures → backoff, then DLT ───────────────────
		errorHandler.addRetryableExceptions(RetryableException.class, java.net.SocketTimeoutException.class);

		// ── Log each retry attempt ───────────────────────────────────────────────
		errorHandler.setRetryListeners(
				(record, ex, deliveryAttempt) -> log.warn("🔄 Retry attempt {} for key={} on topic={}: {}",
